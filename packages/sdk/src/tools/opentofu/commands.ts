...
const COMMANDS: CommandSpec[] = [
  {
    name: 'init',
    description: 'Initialize an OpenTofu working directory.',
    args: [
  ...
  {
    name: 'fmt',
    description: 'Reformat the configuration files in the current directory.',
    args: [
...
class OpenTofuCommandRegistry implements CommandRegistry {
  readonly toolId = 'opentofu' as const;
  ...
  buildInvocation(name: string, opts: CommandOptions): string[] {
    ...
    if (!spec) throw new Error(`Unknown opentofu command: ${name}`);
...
export const opentofuCommands: CommandRegistry = new OpenTofuCommandRegistry(COMMANDS);
